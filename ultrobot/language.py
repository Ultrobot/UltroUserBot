...
for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "ultrojson")):
        if path.isfile(f"./ultrobot/language/{dil.file.name}"):
            try:
                LANGUAGE_JSON = loads(open(f"./ultrobot/language/{dil.file.name}", "r").read())
            except JSONDecodeError:
                dil.delete()
                remove(f"./ultrobot/language/{dil.file.name}")

                if path.isfile("./ultrobot/language/DEFAULT.ultrojson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./ultrobot/language/DEFAULT.ultrojson", "r").read())
                else:
                    raise Exception("Your language file is invalid")
        else:
            try:
                DOSYA = dil.download_media(file="./ultrobot/language/")
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./ultrobot/language/{LANGUAGE}.ultrojson"):
        try:
            LANGUAGE_JSON = loads(open(f"./ultrobot/language/{LANGUAGE}.ultrojson", "r").read())
        except JSONDecodeError:
