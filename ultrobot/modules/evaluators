...
from ultrobot import CMD_HELP, BOTLOG, BOTLOG_CHATID
...
async def evaluate(query):
    ...
    if (expression in ("ultrobot.session", "config.env")) or (expression == 'env'):
...
async def run(run_q):
    ...
    if (code in ("ultrobot.session", "config.env")) or code is 'env':
...
async def terminal_runner(term):
    ...
    if command in ("ultrobot.session", "config.env", "env"):
