...
from ultrobot import CMD_HELP, bot, LOGS, CLEAN_WELCOME, BOTLOG_CHATID
...
async def goodbye_to_chat(event):
    try:
        from ultrobot.modules.sql_helper.goodbye_sql import get_current_goodbye_settings
        from ultrobot.modules.sql_helper.goodbye_sql import update_previous_goodbye
    except:
        return
...
async def save_goodbye(event):
    try:
        from ultrobot.modules.sql_helper.goodbye_sql import add_goodbye_setting
    except:
...
async def del_goodbye(event):
    try:
        from ultrobot.modules.sql_helper.goodbye_sql import rm_goodbye_setting
    except:
        await event.edit("`SQL dışı modda çalışıyor!`")
        return
