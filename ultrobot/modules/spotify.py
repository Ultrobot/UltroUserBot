...
from ultrobot import (BIO_PREFIX, BOTLOG, BOTLOG_CHATID, CMD_HELP, DEFAULT_BIO,
    ...
    while SPOTIFYCHECK:
        try:
            ...
            if song != oldsong and artist != oldartist:
                ...
                spobio = "@UltroUserBot 🎧: " + artist + " - " + song
                try:
...
def get_spotify_info(TIME=5):
    try:
        ...
        if len(item['album']['images']) > 0:
            ...
            if path.exists("@UltroUserBot-Spotify.jpg"):
                remove("@UltroUserBot-Spotify.jpg")          
            try:
                r = get(str(item['album']['images'][0]['url']))
                with open("@UltroUserBot-Spotify.jpg", 'wb') as f:
                    f.write(r.content)    

                with open('UltroUserBot-Spotify.jpg', 'rb') as f:
