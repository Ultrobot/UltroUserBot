...
import re
from ultrobot import BOTLOG, BOTLOG_CHATID, CMD_HELP, BLACKLIST_CHAT
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            try:
                from ultrobot.modules.sql_helper.filter_sql import get_filters
...
async def genelfilter(event):
    try:
        from ultrobot.modules.sql_helper.filter_sql import add_filter
    except AttributeError:
    ...
    if event.chat_id == -1001197341555 or event.chat_id == -1001431607309:
        await event.edit("`Kusura Bakma Ama Resmi Ultro Grubunda Genel Filtre Eklenemez!`")
        return
    else:
...
async def add_new_filter(new_handler):
    ...
    if new_handler.chat_id == -1001197341555 or new_handler.chat_id == -1001431607309:
        await new_handler.edit("`Kusura Bakma Ama Resmi Ultro Grubunda Filtre Eklenemez!`")
        return
    else:
...
async def remove_a_genel(r_handler):
    ...
    try:
        from ultrobot.modules.sql_helper.filter_sql import remove_filter
    except AttributeError:
