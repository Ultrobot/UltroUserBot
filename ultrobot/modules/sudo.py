...
ultrosudo = os.environ.get("SUDO_ID", None)
...
async def get_user(event):
    ...
    ultrotext = replied_user.user.id
    return ultrotext
...
async def sudosil(event):
  ...
  if event.is_reply:
    ...
    op = re.search(str(id), str(ultrosudo))
    if op:
      i = ""
      esudo = ultrosudo.split(" ")
      esudo.remove(str(id))
      i += str(esudo)
      x = i.replace("[", "")
      xx = x.replace("]", "")
...
async def get_user(event):
    ...
    ultrot = replied_user.user.id
    return ultrot
    
@register(incoming=True, from_users=SUDO_ID, pattern="^.salive$")
async def _(q):
    await q.client.send_message(q.chat_id,"`Sudom ❤️ UltroUserBot Çalışıyor...`")
