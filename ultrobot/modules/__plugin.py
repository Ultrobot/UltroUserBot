...
from ultrobot import CMD_HELP, bot, tgbot, PLUGIN_CHANNEL_ID, PATTERNS, BOTLOG, BOTLOG_CHATID, ASISTAN, MYID, DEFAULT_NAME
...
async def pport(event):
    ...
    if re.search(borg1, dosy):
        ...
        degistir = degistir.replace("from ultrobot.utils import admin_cmd", "from ultrobot.events import register")
        degistir = re.sub(r"(from uniborg).*", "from ultrobot.events import register", degistir)
...
async def pins(event):
    ...
    await event.edit(LANG["DOWNLOADING"])
    edizin = f"./ultrobot/modules/{reply_message.file.name}"
  ...
  #  plugins = await event.client.get_messages('@ultroplugin', limit=None, search=dosyaAdi, filter=InputMessagesFilterDocument)

  #  if len(plugins) == 0:
   #     await event.edit('🍕 `Pizzamı yemeye devam edeceğim. Bu bir Ultro Plugini değil!`')
 #       return

    dosya = await event.client.download_media(reply_message, "./ultrobot/modules/")
    ...
    try:
        ...
        return os.remove("./ultrobot/modules/" + dosya)
    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Modül Başarıyla Yüklendi!**\n__Modülün Kullanımını Öğrenmek İçin__ `.ultro {cmdhelp}` __yazın.__')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Modül Başarıyla Yüklendi**\n__Modülün  Kullanımını Öğrenmek İçin__ `.ultro {dosyaAdi}` __yazın.__')
...
async def ptest(event):
    ...
    await event.edit(LANG["DOWNLOADING"])
    if not os.path.exists('./ultrobot/temp_plugins/'):
        os.makedirs('./ultrobot/temp_plugins')
    dosya = await event.client.download_media(reply_message, "./ultrobot/temp_plugins/")
    ...
    try:
        ...
        await event.edit(f"{LANG['PLUGIN_BUGGED']} {e}`")
        return os.remove("./ultrobot/temp_plugins/" + dosya)
    plugin = await event.get_reply_message()
    dosy = open(dosya, "r").read()
    for S in SECURİTY:
     if re.search(S, dosy):
...
async def psend(event):
    ...
    if os.path.isfile(f"./ultrobot/modules/{modul}.py"):
        await event.client.send_file(event.chat_id, f"./ultrobot/modules/{modul}.py", caption=LANG['ULTRO_PLUGIN_CAPTION'])
        await event.delete()
    else:
...
async def premove(event):
    ...
    async for message in event.client.iter_messages(PLUGIN_CHANNEL_ID, filter=InputMessagesFilterDocument, search=modul):
        ...
        try:
            os.remove(f"./ultrobot/modules/{message.file.name}")
...
async def pinsasistan(ups):
    ...
    dosya = await ups.client.download_media(reply_message, "./ultrobot/modules/")
