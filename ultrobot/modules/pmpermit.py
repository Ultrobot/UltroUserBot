...
from ultrobot import (COUNT_PM, CMD_HELP, BOTLOG, BOTLOG_CHATID, DEFAULT_NAME,
...
async def permitpm(event):
    ...
    if PM_AUTO_BAN:
        ...
        if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
            ...
            try:
                from ultrobot.modules.sql_helper.pm_permit_sql import is_approved
                from ultrobot.modules.sql_helper.globals import gvarstatus
...
async def auto_accept(event):
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from ultrobot.modules.sql_helper.pm_permit_sql import is_approved
            from ultrobot.modules.sql_helper.pm_permit_sql import approve
        except AttributeError:
            return
...
async def notifoff(noff_event):
    ...
    try:
        from ultrobot.modules.sql_helper.globals import addgvar
    except AttributeError:
...
async def notifon(non_event):
    ...
    try:
        from ultrobot.modules.sql_helper.globals import delgvar
    except:
...
async def disapprovepm(disapprvpm):
    try:
        from ultrobot.modules.sql_helper.pm_permit_sql import dissprove
    except:
...
async def blockpm(block):
    ...
    if block.reply_to_msg_id:
        ...
        if replied_user.id in BRAIN_CHECKER or replied_user.id in WHITELIST:
            await block.edit(
                "`Oups, Hata! Beni Bağışla Ama Bunu UltroUserBot Yetkilisine Yapamam!!`"
            )
            return
