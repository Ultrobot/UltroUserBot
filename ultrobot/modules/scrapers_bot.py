...
from ultrobot import bot, CMD_HELP
from time import sleep
import os
from telethon.tl.types import MessageMediaPhoto
import asyncio
from ultrobot.modules.admin import get_user_from_event
...
def MemeYap (Resim, Text, FontS = 40, Bottom = False, BottomText = None):
    ...
    Font = ImageFont.truetype("./ultrobot/fonts/impact.ttf", FontSize)
...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./UltroSticker.png"):
                os.remove("./UltroSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("UltroSticker.png", "png")
            Resim = "UltroSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' UltroSticker.png")
            os.remove(sticker)
            Resim = "UltroSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} UltroThumb.jpg")
            os.remove(Resim)
            Resim = 'UltroThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./ultromeme.png"):
            os.remove("./ultromeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./ultromeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
