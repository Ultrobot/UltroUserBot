...
from ultrobot import CMD_HELP, HEROKU_APIKEY, HEROKU_APPNAME, UPSTREAM_REPO_URL, ASISTAN, MYID, AUTO_UPDATE, ULTRO_VERSION, upVer, EMERGENCY
...
async def upstream(ups):
    ...
    if conf != "now" and not force_update:
        UltroVer = int(ULTRO_VERSION.split(".")[1])
        if UltroVer < upVer:
          await ups.edit(f"**Lütfen ultro yöneticileri izin vermeden güncelleme yapmaya çalışma\n Botun bozulabilir\n Güncelleme kanalım :** @UltroUserBot")
          return
...
async def upstream(ups):
    UltroVer = int(ULTRO_VERSION.split(".")[1])
    if UltroVer < upVer:
     await ups.edit(f"**Lütfen ultro yöneticileri izin vermeden güncelleme yapmaya çalişma\n Botun bozulabilir\n Güncelleme kanali :** @UltroUserBot") #CR vERMEYEN NE OLSUN - ByMisakiMey
     return
    await ups.edit(LANG['DETECTING'])
    conf = ups.pattern_match.group(1)
    off_repo = EMERGENCY
