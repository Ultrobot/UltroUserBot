...
from ultrobot import CMD_HELP, BOTLOG, BOTLOG_CHATID, bot, BLACKLIST_CHAT
...
async def kickme(leave):
    if leave.chat_id in BLACKLIST_CHAT:
        return await leave.edit("**Sahibim kusura bakma ama burdan çıkamam resmi ultro grubu burası**,\n\n**Bana gelen yeniliklerden haberdar olman için burada kalmam lazım.**")
...
async def unmute_chat(unm_e):
    ...
    try:
        from ultrobot.modules.sql_helper.keep_read_sql import unkread
...
async def mute_chat(mute_e):
    ...
    try:
        from ultrobot.modules.sql_helper.keep_read_sql import kread
    except AttributeError:
...
async def keep_read(message):
    """ Mute mantığı. """
    try:
        from ultrobot.modules.sql_helper.keep_read_sql import is_kread
    except AttributeError:
        return
    kread = is_kread()
    if kread:
