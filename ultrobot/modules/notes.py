...
from ultrobot import BOTLOG, BOTLOG_CHATID, CMD_HELP
...
async def notes_active(svd):
    ...
    try:
        from ultrobot.modules.sql_helper.notes_sql import get_notes
...
async def remove_notes(clr):
    ...
    try:
        from ultrobot.modules.sql_helper.notes_sql import rm_note
    except AttributeError:
...
async def add_note(fltr):
    ...
    try:
        from ultrobot.modules.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).bot:
            try:
                from ultrobot.modules.sql_helper.notes_sql import get_note
            except AttributeError:
                return
