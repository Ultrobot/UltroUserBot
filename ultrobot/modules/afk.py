...
from ultrobot import (AFKREASON, COUNT_MSG, CMD_HELP, ISAFK, BOTLOG,
...
async def set_afk(afk_e):
    ...
    if string:
        ...
        \n{LANG['REASON']}: `{string}`")
        await afk_e.client(UpdateProfileRequest(about='Sahibim Şuan #AFK @Ultrouserbot ✨')) #BUNU KULLANIPTA CR VERMEYENİN ANASINI SİKİM TEŞEKKÜRLER / ByMisakiMey
    else:
        await afk_e.edit(LANG['IM_AFK'])
        await afk_e.client(UpdateProfileRequest(about='Sahibim Şuan #AFK @Ultrouserbot ✨'))
    SON_GORULME = time()
    if BOTLOG:
