...
from ultrobot import CMD_HELP, BOTLOG_CHATID
...
async def on_snip(event):
    """ Snip mantığı. """
    try:
        from ultrobot.modules.sql_helper.snips_sql import get_snip
...
async def on_snip_save(event):
    ...
    try:
        from ultrobot.modules.sql_helper.snips_sql import add_snip
...
async def on_snip_list(event):
    ...
    try:
        from ultrobot.modules.sql_helper.snips_sql import get_snips
    except AttributeError:
...
async def on_snip_delete(event):
    ...
    try:
        from ultrobot.modules.sql_helper.snips_sql import remove_snip
    except AttributeError:
        await event.edit("`SQL dışı modda çalışıyor!`")
        return
