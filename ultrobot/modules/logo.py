...
from ultrobot import DEFAULT_NAME
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("ultrobot/fonts/*")
    ...
    if os.path.exists(fayladi):
        await event.client.send_file(
            event.chat_id,
            file=fayladi,
            caption=f"[UltroUserBot](https://t.me/ultrouserbot) **{DEFAULT_NAME}** için logo hazırladı.",
            force_document=True,
        )
        os.remove(fayladi)
        await event.delete()
    ...
    if os.path.exists(font_):
        if not font_.startswith("ultrobot/fonts/"):
            os.remove(font_)

Help = CmdHelp('logo')
Help.add_command('logo', '<yazı>', '@UltroUserBot sizin için logo hazırlar.')
Help.add_info('@UltroUserBot - @ByMisakiMey ')
Help.add() 
