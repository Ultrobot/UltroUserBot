...
from . import BRAIN_CHECKER, LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, ULTRO_VERSION, PATTERNS, DEFAULT_NAME, BOT_TOKEN
from .modules import ALL_MODULES
from .asisstant.modules import ALL_MODULE
import ultrobot.modules.sql_helper.mesaj_sql as MSJ_SQL
import ultrobot.modules.sql_helper.galeri_sql as GALERI_SQL
...
ALIVE_MSG = [
"""
✨ `UltroUserBot sahibinin emirlerine hazır...`
...
UNAPPROVED_MSG = ("`Hey olduğun yerde kal,!👨‍💻 Ben Ultro. Endişelenme!\n\n`"
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        CmdHelp = ultrobot.cmdhelp.CmdHelp(dosyaAdi, False)
        ...
        for Command in Pattern:
            ...
            Ultropy = re.search('\"\"\"ULTROPY(.*)\"\"\"', FileRead, re.DOTALL)
            if not Ultropy == None:
                Ultropy = Ultropy.group(0)
                for Satir in Ultropy.splitlines():
...
try:
    bot.start()
    idim = bot.get_me().id
    ultrobl = requests.get('https://raw.githubusercontent.com/ultrobot/database/master/blacklist.json').json()
    if idim in ultrobl:
        bot.send_message("me", f"`❌ Ultro yöneticileri sizi bottan yasakladı! Bot kapatılıyor...`")
        LOGS.error("Ultro yöneticileri sizi bottan yasakladı! Bot kapatılıyor...")
        bot.disconnect()
    ...
    PLUGIN_MESAJLAR_TURLER = ["alive", "afk", "kickme", "pm", "dızcı", "mute", "approve", "disapprove", "block"]
    ...
    if not PLUGIN_CHANNEL_ID == None:
        ...
        for plugin in bot.iter_messages(KanalId, filter=InputMessagesFilterDocument):
            if plugin.file.name and (len(plugin.file.name.split('.')) > 1) \
                ...
                if not os.path.exists("./ultrobot/modules/" + plugin.file.name):
                    dosya = bot.download_media(plugin, "./ultrobot/modules/")
                else:
                    LOGS.info("Bu Plugin Onsuzda Yüklüdür " + plugin.file.name)
                    extractCommands('./ultrobot/modules/' + plugin.file.name)
                    dosya = plugin.file.name
                    continue 
                
                try:
                    spec = importlib.util.spec_from_file_location("ultrobot.modules." + Split[0], dosya)
                    ...
                    if os.path.exists("./ultrobot/modules/" + plugin.file.name):
                        os.remove("./ultrobot/modules/" + plugin.file.name)
                    continue
                extractCommands('./ultrobot/modules/' + plugin.file.name)
    else:
...
for module_name in ALL_MODULES:
    imported_module = import_module("ultrobot.modules." + module_name)
if BOT_TOKEN:
 for module_name in ALL_MODULE:
    imported_module = import_module("ultrobot.asisstant.modules." + module_name)    

os.system("clear")

LOGS.info("+===========================================================+")
LOGS.info("|                     ✨Ultro Userbot✨                       |")
LOGS.info("+==============+==============+==============+==============+")
LOGS.info("|                                                            |")
LOGS.info("Botunuz çalışıyor! Herhangi bir sohbete .alive yazarak Test edin."
          " Yardıma İhtiyacınız varsa, Destek grubumuza gelin t.me/UltrobotSupport")
LOGS.info(f"Bot versiyonunuz: Ultro ==> {ULTRO_VERSION}")
