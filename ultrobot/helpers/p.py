def e_(fayl_adi, name, slep, siyahi):
	f = open(f"./ultrouserbot{fayl_adi}.py", "x")
	f.write(f"""from ultrobot.events import register
...
async def _(misaki):
	for i in a:
		...
		sleep({slep})
Help = CmdHelp("ultrouserbot{fayl_adi}")
Help.add_command("{name}", None, "Bu plugin @UltroUserBot tarafından hazırlanmıştır.")
Help.add()
								""")
	return f.close()
...
async def misakimusic(misaki):
    ...
    while netice is False:
            ...
            if rast.description == IFACI:
                ...
                await misaki.client.send_file(misaki.chat_id, yukle, caption="@UltroUserBot sizin için `"+rast.description+" - "+rast.title+"` müziğini seçti iyi dinlemeler. :)")
                await event.delete()
                os.remove(yukle)
                netice = True
Help = CmdHelp("ultrouserbot{fayl_adi}")
Help.add_command("{name}", None, "Bu Plugin @UltroUserBot Tərəfindən Hazırlanmışdır..")
Help.add()
		""".format(
siyahi=siyahi,
name=name,
