...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        level=DEBUG,
        format="[%(asctime)s - %(levelname)s] - @UltroUserBot : %(message)s",
        datefmt='%d-%b-%y %H:%M:%S')
else:
    basicConfig(
        level=INFO,
...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
    if EZZEC == True:
        ...
        heroku_var["UPSTREAM_REPO_URL"] = "https://github.com/UltroBot/UltroUserBot.git"
else:
    app = None
...
UPSTREAM_REPO_URL = "https://github.com/UltroBot/UltroUserBot.git" 
EMERGENCY = "https://github.com/erdewbey/UltroUserBot.git" # Acil durrum için
# Afk mesajların iletilmesi
AFKILETME = sb(os.environ.get("AFKILETME", "True"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///ultro.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@UltroUserBot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "| 🌃 @UltroUserBot Paketi |")

# Userbotu kapatmak için gruplar
BLACKLIST_CHAT = os.environ.get("BLACKLIST_CHAT", None)

if not BLACKLIST_CHAT: #Eğer ayarlanmamışsa Ultro Support grubu eklenir.
...
WHITELIST = get('https://raw.githubusercontent.com/ultrobot/database/master/whitelist.json').json()
...
"""if STRING_SESSION:
    ...
    dosya = open("stringultro.py","w",encoding="utf-8")
...
URL = 'https://raw.githubusercontent.com/ultrobot/database/master/force-surum.check' 
...
URL = 'https://raw.githubusercontent.com/ultrobot/database/master/force-update.check' 
...
DEVS = 1422746074 #developer ayrıcalıkları olacak

PREMIUM = get('https://raw.githubusercontent.com/ultrobot/database/master/premium.json').json() # Premium Üyelerin ID 

ASISTAN = 5372322402 # Bot yardımcısı
...
URL = 'https://raw.githubusercontent.com/ultrobot/database/master/learning-data-root.check'
...
with bot:
    ...
    try:
        bot(JoinChannelRequest("@UltroUserBot"))
        bot(JoinChannelRequest("@UltrobotSupport"))
        bot(JoinChannelRequest("@UltroBot"))
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @UltroUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Ultro açabilirsin; Kanala bak` @UltroUserBot')
            else:
                await event.reply(f'`Tengri save Turks! Ultro working... `')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@UltroUserBot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**En Gelişmiş UserBot!** [Ultro](https://t.me/UltroUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@UltroUserBot",
                    text="""@UltroUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/UltroUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/UltrobotSupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Ultrobot/UltroUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @UltroUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Ultro](https://t.me/UltroUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
